...
parser.add_argument("-c",
                    ...
                    help="Path of output .csv file. If none provided, then no file will be "
                         "written.",
                    type=str, default="virtual_file.csv")
    ...
    for xml_file in glob.glob(path + '/*.xml'):
        ...
        for member in root.findall('object'):
            value = (root.find('filename').text,
                     ...
                     member[0].text,
                     int(round(float(member.find("bndbox")[0].text))),
                     int(round(float(member.find("bndbox")[1].text))),
                     int(round(float(member.find("bndbox")[2].text))),
                     int(round(float(member.find("bndbox")[3].text)))
                     )
